        ...
        <main className="min-h-screen pt-32 px-4 md:px-12 pb-12 bg-[#f0f0f0] text-[#111] flex flex-col justify-between">
            ...
            <div>
                <h1 className="text-6xl md:text-[10vw] leading-[0.8] font-medium tracking-tight mb-12 uppercase">
                    <span dangerouslySetInnerHTML={{ __html: contact.heading || "LET'S START<br/>A PROJECT." }} />
                </h1>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 border-t border-black pt-8">
                ...
                <div className="md:col-span-4">
                    ...
                    <a href={`mailto:${contact.email}`} className="text-2xl md:text-3xl font-medium underline decoration-1 underline-offset-4 hover:text-gray-600 transition-colors break-all block">
                    ...
                    <div className="flex flex-col space-y-2">
                        {contact.socials && contact.socials.length > 0 ? (
                            contact.socials.map((social, idx) => (
                                <a key={idx} href={social.url} target="_blank" className="text-xl border-b border-black/10 pb-2 hover:pl-4 transition-all duration-300 flex justify-between group">
            ...
            <div className="mt-20 border-t border-black/10 pt-12 flex flex-col md:flex-row justify-between items-end gap-8">
                <div className="text-left">
                    <p className="font-mono text-xs text-gray-400 mb-2">{contact.navPrevLabel || "GO BACK"}</p>
                    <Link href="/cv" className="text-xl md:text-2xl font-medium hover:italic transition-all inline-block">
                        {contact.navPrevText || "← Resume"}
                    </Link>
                </div>
                <div className="text-right">
                    <p className="font-mono text-xs text-gray-400 mb-2">{contact.navNextLabel || "SYSTEM ACCESS"}</p>
                    <Link href="/cms" className="text-4xl md:text-6xl font-medium hover:italic transition-all inline-block">
