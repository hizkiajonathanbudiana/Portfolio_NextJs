...
export default function CMSDashboard() {
  ...
  const DashboardCard = ({ number, icon, title, desc, href }) => (
    <Link
      href={href}
      className="group block bg-[#111] border border-[#333] p-6 hover:bg-white hover:text-black transition-all duration-300 relative overflow-hidden"
    >
      <div className="flex justify-between items-start mb-8 relative z-10">
        <span className="font-mono text-xs text-gray-500 group-hover:text-black font-bold">
          {number}
        </span>
      ...
      <h3 className="font-bold text-lg mb-1 relative z-10 uppercase tracking-tight">
        {title}
      </h3>
      <p className="font-mono text-[10px] text-gray-500 group-hover:text-black/60 relative z-10 leading-relaxed">
        {desc}
      </p>
    </Link>
  );
    ...
    <div className="max-w-6xl pb-20">
      ...
      <div className="mb-12 border-b border-[#333] pb-6">
        ...
        <div className="flex items-center gap-3 font-mono text-xs mt-4">
          ...
          {session ? (
            <span className="text-green-500 font-bold uppercase border border-green-900 bg-green-900/10 px-2 py-1">
              ADMINISTRATOR
            </span>
          ) : (
            <span className="text-yellow-500 font-bold uppercase border border-yellow-900 bg-yellow-900/10 px-2 py-1">
              READ ONLY GUEST
            </span>
          )}
        </div>
      ...
      {!session && (
        <div className="bg-yellow-900/10 border border-yellow-600/50 p-6 mb-12 relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-10 text-9xl text-yellow-500 select-none pointer-events-none">
            ⚠
          </div>
          <h3 className="text-yellow-500 font-bold mb-2 uppercase text-sm tracking-widest relative z-10">
            Showcase Mode Active
          </h3>
          <p className="text-gray-400 text-xs font-mono leading-relaxed relative z-10 max-w-2xl">
            You are exploring this CMS in <strong>Showcase Mode</strong>. All
            features (Settings, Projects, Resume, Pages) are unlocked for visual
            inspection.
            <br />
            <span className="opacity-50">
              Write operations (Save/Delete) will be simulated and rejected by
              the server security layer.
            </span>
          </p>
        </div>
      )}
